...
fun AbsBanner(
	...
	Column(
		...
		Text(
			text = stringResource(R.string.advance_balance_booking),
			style = textMediumNoto14Sp(Neutral90)
		)
		if (!abs?.lastMovedSchemeAmount.isNullOrEmpty()) {
			Text(
				text = stringResource(
					R.string.adjust_remaining_balance,
					abs?.lastMovedSchemeAmount.orEmpty()
				),
		...
		Row(Modifier.padding(top = 18.dp)) {
			val amount =
				remember(abs?.amount) { abs?.amount?.orZero().toString().getAmountInRupees() }
			...
			if (abs?.amount.orZero() > 0) {
				Row(modifier = Modifier.clickable {
					...
					Text(
						...
						style = textMediumNoto16Sp(Neutral70)
					)
					Image(
